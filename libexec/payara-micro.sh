...
if [ "${COMMAND}" == "install" ]; then
    if [ -z "${VERSION}" ]; then
        VERSION=`curl -L -s 'https://search.maven.org/solrsearch/select?q=g:fish.payara.extras+AND+a:payara-micro' | perl -wn -e 'print $1 if /"latestVersion":"([^"]+)"/'`
    fi
    ...
    for VERSION in `curl -L -s 'https://search.maven.org/solrsearch/select?q=g:fish.payara.extras+AND+a:payara-micro&core=gav' | perl -wp -e 's!,!,\n!g' | perl -wnl -e 'print "$1 " if /"v":"([^"]+)"/' | sort | grep -vEi 'alpha|beta'`
    do
        if [ "${CURRENT_VERSION}" == "${VERSION}" ]; then
            echo "  ${CURRENT_VERSION} [current]"
        else
